...
public class WordCount extends Configured implements Tool{
	...
	public int run(String[] args) throws Exception {
		if (args.length != 1) {
			System.err.printf("Usage: %s needs one arguments <output> files\n",
		...
		job.setJobName("CityCounter");
		
		JobConf conf = new JobConf(getConf(), WordCount.class);
		conf.setInputFormat(DBInputFormat.class);
		DBConfiguration.configureDB(conf,
		    "com.mysql.jdbc.Driver",
		    "jdbc:mysql://frodo.bentley.edu/world");
		String [] fields = { "Name", "CountryCode", "District", "Population" };
		DBInputFormat.setInput(conf, MyRecord.class, "city",
		    null /* conditions */,  "Name", fields);
